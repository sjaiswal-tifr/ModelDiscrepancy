...
fixed_param = None  # fix T_kink, a_low, a_high
# ======================================================================================
...
model_param_bounds = np.array([[ 0.0 ,  0.4 ],
                               [ 0.1 ,  0.5 ]])
...
def get_exp_dict(experimental_data, observables):
    ...
    # Scale the x_values (p_T or centrality) so that 0 <= x_values <= 1. This is to make prior specification 
    # easy and avoid numerical issues in generating positive definite covariance kernel when r>1 .
    # x_values = x_values/max(x_values)
...
def model_predict(theta):
    ...
    fixed = np.atleast_1d(fixed_param)
    theta_full = np.concatenate((fixed, theta))
    theta = np.atleast_2d(theta_full)
